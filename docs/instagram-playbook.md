# Sisi Club — Instagram & marketing playbook

The canonical production rules for all Sisi Club social/marketing imagery. Claude Code:
read this file top to bottom before generating any post image, story, or applying a
photo filter. Match it exactly — the brand's whole appeal is that every post looks
like it came from the same quiet hand.

The as-delivered client kit lives frozen in [`_handoff/instagram-kit/`](../_handoff/instagram-kit/)
(open `Instagram-Kit.html` in a browser to see the whole system rendered). This file is
the living version — if they ever disagree, **this file wins**.
...
- **Filtered photos** — existing photography re-graded to the brand look (see §12).
...
> Note: the reference kit in `_handoff/instagram-kit/` shows its post mockups at 1:1 —
...
**Carousel frames** (see the `Legacy of Sisi` example in the kit — 4 frames, 1080 × 1350 each):
...
Treat every deliverable as **one fixed-pixel HTML canvas → one JPG**.
...
1. **Build** a standalone HTML file whose root element is exactly the target size
   ...
   Google Fonts link in §6. Reference photos: `_handoff/instagram-kit/img/` (kit
   photography, PNG) and `public/images/` (site photography, WebP).
...
3. **Save as JPG, quality ~90.** Naming: `NN-slug.jpg` (`01-cover.jpg`, `02-the-walk.jpg`).
   Feed exports go in `export/instagram/feed/`; stories in `export/instagram/stories/`.
...
capture (see the carousel source in the kit). Capture each frame in turn.
...
await el.screenshot({ path: 'export/instagram/feed/01-cover.jpg', quality: 90, type: 'jpeg' });
await browser.close();
```
...
- Real reference photos live in `_handoff/instagram-kit/img/` (hero-walk, lakeside, dog-hug,
  summit, picnic, lauenen, fivelakes, sisi-portrait) and `public/images/`. Swap in new
  photography that matches this mood.
- No alcohol, no cold-weather styling, no stocky/corporate posing.

---

## 12. Photo treatment & filters ("put the Sisi filter on it")

When asked to **filter**, **grade**, or **make a photo match the feed**, apply one of the
presets below. The goal is the §11 mood: painterly, warm, slightly sun-faded, earthy —
a photo that could sit next to any existing post without looking edited.

**Golden rules**

- Filters are **subtle**. At a glance the photo should look unfiltered — just warmer,
  calmer, and slightly faded. If it obviously "has a filter," dial it back.
- **Warm only, never cold.** No blue/teal casts, no cool white balance.
- **No** heavy vignettes, HDR/clarity crunch, high contrast, oversharpening, or
  Instagram's built-in in-app filters.
- The bottom **scrim** on photo posts (§7) is a layout element, not a filter — apply the
  filter to the photo first, then the scrim on top when there's text.
- Deliver filtered photos as JPG quality ~90, same pixel size as the input unless a
  post-format crop (§4) was requested.

**Presets** — CSS `filter` values are for the HTML-canvas route (§9); the ImageMagick
command is for filtering a flat photo directly (`brew install imagemagick` if `magick`
is missing).

1. **Sisi Standard** — the default; use when the request is just "filter this".
   - CSS: `filter: sepia(0.12) saturate(0.88) brightness(1.03) contrast(0.94);`
     plus a cream wash overlay: a full-bleed layer of `#e8d6ad` at `opacity:0.10` with
     `mix-blend-mode:soft-light`.
   - ImageMagick: `magick in.jpg -modulate 103,88 -fill '#e8d6ad' -colorize 6% +level 3%,99% out.jpg`

2. **Sun-faded** — stronger; for off-palette photos (harsh colours, cold light) that
   need pulling into the grid.
   - CSS: `filter: sepia(0.20) saturate(0.72) brightness(1.05) contrast(0.88);`
     cream wash at `opacity:0.14`.
   - ImageMagick: `magick in.jpg -modulate 105,72 -fill '#e8d6ad' -colorize 10% +level 6%,97% out.jpg`

3. **Golden hour** — adds warmth/glow to flat midday shots.
   - CSS: `filter: sepia(0.16) saturate(0.95) brightness(1.02) contrast(0.97);`
     gold wash: `#c9b48a` at `opacity:0.12`, `mix-blend-mode:soft-light`.
   - ImageMagick: `magick in.jpg -modulate 102,95 -fill '#c9b48a' -colorize 8% +level 2%,99% out.jpg`

4. **Heritage** — archival Empress-Sisi / namesake content only. The one preset allowed
   to go monochrome.
   - CSS: `filter: grayscale(1) sepia(0.35) brightness(1.02) contrast(0.92);`
   - ImageMagick: `magick in.jpg -colorspace Gray -fill '#9a7f4f' -tint 30 +level 4%,98% out.jpg`

**Verify:** open the result next to an existing feed image (kit `img/` or the rendered
`Instagram-Kit.html`) and check it reads as the same family — warm highlights toward
cream `#F6F0E6`, shadows toward brown `#463F36`, greens muted toward sage.
